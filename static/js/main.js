    ...
    function renderDashboard(data) {
        ...
        if (weather.current) {
            html += `
                <div class="row mb-3">
                    <div class="col">
                        <strong>Current Temperature:</strong> ${weather.current.temperature_2m}°${units === "celsius" ? "C" : "F"}
                    </div>
                    <div class="col">
                        <strong>Wind:</strong> ${weather.current.wind_speed_10m} ${units === "celsius" ? "km/h" : "mph"} (${weather.current.wind_direction_10m}°)
                    </div>
                    <div class="col">
                        <strong>Humidity:</strong> ${weather.current.humidity_2m}%
                    </div>
                    <div class="col">
        ...
        if (forecast) {
            ...
            if (forecast.time) html += `<th>Time</th>`;
            if (forecast.temperature_2m) html += `<th>Temp (${units === "celsius" ? "°C" : "°F"})</th>`;
            if (forecast.wind_speed_10m || forecast.wind_speed_10m_max) html += `<th>Wind</th>`;
            if (forecast.wind_direction_10m || forecast.wind_direction_10m_dominant) html += `<th>Wind Dir</th>`;
            if (forecast.precipitation || forecast.precipitation_sum) html += `<th>Precipitation</th>`;
            if (forecast.precipitation_probability || forecast.precipitation_probability_max) html += `<th>Precip Prob</th>`;
            if (forecast.humidity_2m || forecast.humidity_2m_max) html += `<th>Humidity</th>`;
            if (forecast.uv_index || forecast.uv_index_max) html += `<th>UV Index</th>`;
            html += `</tr></thead><tbody>`;
            for (let i = 0; i < (forecast.time || []).length; i++) {
                html += `<tr>`;
                if (forecast.time) html += `<td>${forecast.time[i]}</td>`;
                ...
                if (forecast.humidity_2m) html += `<td>${forecast.humidity_2m[i]}</td>`;
                ...
                if (forecast.humidity_2m_max) html += `<td>${forecast.humidity_2m_max[i]}</td>`;
...
function renderDashboard(data) {
    ...
    if (weather.current) {
        ...
        html += `
            <div class="row mb-3 align-items-center">
                <div class="col-auto">
                    <img src="${icon}" alt="${desc}" title="${desc}" style="width:48px;height:48px;">
                </div>
                <div class="col">
    ...
    if (forecast && forecast.time) {
        ...
        for (let i = 0; i < forecast.time.length; i++) {
            ...
            html += `<td>${icon ? `<img src="${icon}" alt="${desc}" title="${desc}" style="width:32px;height:32px;">` : ""}</td>`;
